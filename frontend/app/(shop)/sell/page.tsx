...
export default function SellLandingPage() {
  ...
  const becomeSeller = () => {
    ...
    // Logged-in buyer/user — go to seller setup/onboarding page
  ...
  return (
    <div className="pt-24 pb-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      ...
      <button onClick={becomeSeller} disabled={loading} className="px-10 py-4 rounded-2xl text-base font-semibold inline-flex items-center justify-center gap-3 disabled:opacity-60 transition-all" style={{ background: 'linear-gradient(135deg,#C9A84C,#E8CC7A)', color: '#06040E', boxShadow: '0 4px 20px rgba(201,168,76,0.3)' }}>
        {u?.is_seller ? <>Go to Seller Dashboard <ArrowRight size={18} /></> : user ? <>Become a Seller <ArrowRight size={18} /></> : <>Create Account & Start Selling <ArrowRight size={18} /></>}
      </button>
      {u?.is_seller && <p className="text-sm mt-4" style={{ color: '#25D366' }}><CheckCircle size={14} className="inline mr-1" /> You're already a seller — click above to go to your dashboard.</p>}
