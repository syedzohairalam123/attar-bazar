...
export function ProductDetailClient({ perfume, mode }: { perfume: Perfume; mode: 'gallery' | 'actions' }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4"><span className="text-sm" style={{ color: '#A89F8F' }}>Quantity:</span><div className="flex items-center gap-2 glass-card rounded-xl p-1"><button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-9 h-9 flex items-center justify-center rounded-lg transition-all" style={{ color: '#A89F8F' }}><Minus size={16} /></button><span className="w-8 text-center font-semibold" style={{ color: '#F5F0E8' }}>{qty}</span><button onClick={() => setQty(q => Math.min(perfume.quantity, q + 1))} disabled={qty >= perfume.quantity} className="w-9 h-9 flex items-center justify-center rounded-lg transition-all disabled:opacity-30" style={{ color: '#A89F8F' }}><Plus size={16} /></button></div></div>
      ...
      <div className="flex gap-3"><button onClick={() => setLiked(!liked)} className="flex-1 py-3 rounded-xl border text-sm font-medium flex items-center justify-center gap-2 transition-all" style={liked ? { borderColor: 'rgba(239,68,68,0.5)', background: 'rgba(239,68,68,0.1)', color: '#f87171' } : { borderColor: 'rgba(201,168,76,0.3)', color: '#C9A84C' }}><Heart size={16} fill={liked ? 'currentColor' : 'none'} />{liked ? 'Saved' : 'Save'}</button><button onClick={share} className="flex-1 py-3 rounded-xl border text-sm font-medium flex items-center justify-center gap-2 transition-all" style={{ borderColor: 'rgba(201,168,76,0.3)', color: '#C9A84C' }}><Share2 size={16} /> Share</button></div>
      <div className="glass-card rounded-2xl p-4"><p className="text-xs uppercase tracking-wider mb-3" style={{ color: '#A89F8F' }}>Pay Via</p><div className="flex flex-wrap gap-2">{[{ l: 'JazzCash', bg: '#CC0000' }, { l: 'EasyPaisa', bg: '#00A651' }, { l: 'Bank Transfer', bg: '#1d4ed8' }, { l: 'Cash on Delivery', bg: '#4b5563' }].map(m => (<div key={m.l} className="px-3 py-1.5 rounded-lg text-xs font-bold text-white" style={{ background: m.bg }}>{m.l}</div>))}</div></div>
    </div>
  )
}
